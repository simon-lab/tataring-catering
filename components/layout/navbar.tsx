...
export function Navbar() {
  ...
  const navLinks = NAV_LINKS.map((l) => ({ href: l.href, label: t(l.key) }));
      ...
      >
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href="/" className="flex flex-col leading-none shrink-0">
            <span className="font-heading text-xl text-primary tracking-wide">TATARING</span>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname.startsWith(link.href) ? "text-primary font-semibold" : "text-foreground/65"
                )}
              >
                {link.label}
              </Link>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <LanguageToggle />
            <Link href="/order" className={buttonVariants({ size: "sm" })}>
              {t("nav.order")}
            </Link>
          </div>
