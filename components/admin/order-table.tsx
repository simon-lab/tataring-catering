...
export function OrderTable({ orders }: { orders: Order[] }) {
  ...
  const handleNoteSave = (id: string) => {
    ...
    setNoteSaved(null);
    startTransition(async () => {
      const result = await updateOrderNote(id, noteValues[id] ?? "");
      if (result.error) {
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            disabled={isPending}
                            className={cn(
                              "flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-medium transition-colors disabled:opacity-50",
                              noteSaved === order.id
                          ...
                          >
                            ...
                            {noteSaved === order.id ? "Tersimpan!" : "Simpan"}
                          </button>
                        </div>
                      </div>
                    </div>
